...
int main()
{
	...
	ifstream source;                    
    source.open("datahighdimension.txt", ios_base::in); 
	ofstream file;                    
    file.open("datahighdimension.txt",ios::app);
	std::string line;
	...
	for(int i=0;i<noofpoints-1;++i)
	{
		...
		{	
			for(int dim1=0;dim1<dim-1;++dim1)
			{
				for(int dim2=dim1+1;dim2<dim;++dim2)	
				{
					if(points[j][dim]==clusterno)
					{	
						cov[clusterno][dim1][dim2]+=(points[i][dim1]-points[j][dim1])*(points[i][dim2]-points[j][dim2]);
						cov[clusterno][dim2][dim1]=cov[clusterno][dim1][dim2];
					}	
				}	
			}	
		}
	...
	{	
		for(int dim1=0;dim1<dim-1;++dim1)
			{
				for(int d2=dim1+1;d2<dim;++d2)	
				{
						
					cov[i][dim1][d2]/=pincluster[i];
					cov[i][d2][dim1]/=pincluster[i];
